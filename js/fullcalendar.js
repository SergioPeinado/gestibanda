...
TimeGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		return '<a cod="' + event.Codigo + '" class="' + classes.join(' ') + '"' +	// MODIFICADO
			(event.url ?
				' href="' + htmlEscape(event.url) + '"' :
				''
				) +
			...
			'>' +
				'<div class="fc-content"><span><i class="fa fa-search pull-right"></i></span>' +	// MODIFICADO
					(timeText ?
						'<div class="fc-time"' +
...
function Calendar_constructor(element, overrides) {
	...
	function prev() {
		...
		cargaCitas(moment(date).format("DD-MM-YYYY"),true);		// MODIFICADO!!
		renderView();
	}
...
function Header(calendar, options) {
	...
	function updateTitle(text) {
		el.find('h2').text("Citas del día " + text);	//MODIFICADO
	}
